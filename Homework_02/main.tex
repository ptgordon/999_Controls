...
\answer{The difference in velocities between $m_1$ and $m_2$ will reach a steady state because $b_2$ scales with velocity. Since there is no friction, the output $x_2(t)$ will not decay.}

\clearpage
\problem{1.C}{Modified cruise control}
\answer{
    Equation 2.4 from Example 2.1 in the text book reads: 
    \[\dot{v} + \frac{b}{m}v = \frac{u}{m}\]

    Substituting $u=K(v_r-v)$, the equation becomes:
    \[\dot{v} + \frac{b}{m}v = \frac{K(v_r-v)}{m}\]
}

\answerfigure[0.4\linewidth]{figures/1_C_schematic.pdf}{Schematic diagram of car cruise control model}

\begin{symboltable}{Signal}{Signals of Problem 1.B.}
    \sym{v(t)}{m}{Measured velocity of car}
    \sym{v_r(t)}{m}{Commanded velocity of car}
\end{symboltable}

\begin{symboltable}{Parameter}{Parameters of Problem 1.B.}
    \sym{m}{Kg}{mass of car}
    \sym{b}{N-s/m}{Outer spring stiffness, wall to $m$}
    \sym{K}{N-s/m}{gain constant}
\end{symboltable}
...
\answer{
    ...
    \begin{align*}
        f_{net} &= f_K - f_b \\
        m\dot{v}(t) &= K(v_r(t) - v(t)) - bv(t) \\
    \end{align*}

    Standard form:
    \begin{align*}
        Kv_r(t) &= m\dot{v}(t) + Kv(t) + bv(t) \\
        Kv_r(t) &= m\dot{v}(t) + (K+b)v(t) \\
    \end{align*}
    ...
    Transfer function:
    \[ G_{V(s)} = \frac{V(s)}{V_r(s)} = \frac{K}{ms + (K+b)} \]

    Units:
    \[ \frac{\frac{N \cdot s}{m}}{Kg \cdot s^{-1} + \frac{N \cdot s}{m} + \frac{N \cdot s}{m} }\]
    \[ \frac{\frac{N \cdot s}{m}}{\frac{N \cdot s^{\cancel{2}}}{m} \cdot \cancel{s^{-1}} + \frac{N \cdot s}{m} + \frac{N \cdot s}{m} }\]

    \matlabcode{hw_02_1C.m}

    \answerfigure[0.8\linewidth]{figures/hw_02_1C.pdf}{Unit step response of $G_{V(s)}$ for $K=1000$, $K=5000$, and $K=10000$.}

    \answer{No value of K will cause issues with stability or oscillations, but the tradeoff is excessive acceleration against accuracy of output. Given the graphs, a value between 1000 and 5000 would probably get close enough to the target without accelerating too quickly.}
}

\clearpage

\problem{2}{Transfer function of a circuit}
\answerfigure[0.8\linewidth]{figures/2_Schematic.png}{The RL circuit of Problem 2, with input $v_A(t)$ and output $v_o(t)$.}

\begin{symboltable}{Signal}{Signals of Problem 1.B.}
    \sym{v_A(t)}{V}{Input Voltage}
    \sym{v_o(t)}{V}{Position of mass $m_2$}
    \sym{v_{R1}(t)}{V}{Voltage Across R1}
    \sym{i_{R1}(t)}{A}{Current through R1}
    \sym{i_{R2}(t)}{A}{Current through R2}
    \sym{i_{L1}(t)}{A}{Current through L1}
\end{symboltable}

\begin{symboltable}{Parameter}{Parameters of Problem 1.B.}
    \sym{R_1}{Ohm(V/A)}{Resistor}
    \sym{R_2}{Ohm(V/A)}{Resistor}
    \sym{L_1}{H(V-s/A)}{Resistor}
\end{symboltable}
...
\answer{
    ...
    \begin{align*}
        v_A(t) &= v_{R_1}(t) + v_o(t) \\
        i_{R_1}(t) &= i_{L_1}(t) + i_{R_2}(t) \\
        v_o(t) &= L_1\dot{i}_{L_1}(t)
    \end{align*}
    ...
    \begin{align*}
        i_{R_1}(t) &= i_{L_1}(t) + i_{R_2}(t) \\
        i_{R_2}(t) &= \frac{v_o(t)}{R2} \\
        i_{R_1}(t) &= \frac{v_A(t)-v_o(t)}{R1} \\
        \frac{v_A(t)-v_o(t)}{R1} &= i_{L_1}(t) + \frac{v_o(t)}{R2} \\
        v_o(t) &= L_1\dot{i}_{L_1}(t)
    \end{align*}

    Move things around to get to standard form:

    \begin{align*}
        R_2(v_A(t)-v_o(t)) &= R_1R_2i_{L_1}(t) + R_1v_o(t) \\
        R_2v_A(t) &= R_1v_o(t) + R_2v_o(t) + R_1R_2i_{L_1}(t) \\
        \\
        v_o(t) &= L_1\dot{i}_{L_1}(t)
    \end{align*}
    ...
    \begin{align*}
        R_2V_A(s) &= (R_1 + R_2)V_o(s) + R_1R_2I_{L_1}(s) \\
        V_o(s) &= L_1sI_{L_1}(s)
    \end{align*}
    ...
    \begin{align*}
        V_o(s) &= L_1sI_{L_1}(s) \\
        I_{L_1}(s) &= \frac{1}{L_1s}V_o(s) \\
        \\
        R_2V_A(s) &= (R_1 + R_2)V_o(s) + \frac{R_1R_2}{L_1s}V_o(s) \\
        R_2L_1sV_A(s) &= (L_1s(R_1 + R_2) + R_1R_2)V_o(s) \\
        G_{Ao}(s) = \frac{V_o(s)}{V_A(s)} &= \frac{R_2L_1s}{(R_1 + R_2)L_1s + R_1R_2}
    \end{align*}

    Check Units:

    \begin{align*}
        \frac{\frac{V}{A}\frac{V\cdot \cancel{s}}{A}\cancel{s^{-1}}}{\frac{V}{A}\frac{V\cdot \cancel{s}}{A}\cancel{s^{-1}} + \frac{V}{A}\frac{V}{A}}
    \end{align*}

    The Gain is a voltage over a voltage, so all units should cancel, as they do with this solution.
}

\clearpage
\problem{3}{Matlab}

\matlabcode{hw_02_3A.m}
\answerfigure[0.6\linewidth]{figures/hw_02_3A.pdf}{Unit step response of $G_{V(s)}$ for $b=5$, $b=50$, and $b=500$.}
